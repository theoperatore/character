...
test('Update character traits', () => {
  ...
  expect(before.get('flaws')).not.toEqual(
    after.get('flaws'),
    'flaws should have changed'
  );
  ...
  expect(before.get('personalityTraits')).toEqual(
    ...
    'personality should have not changed'
  );
  expect(before.get('ideals')).toEqual(
    after.get('ideals'),
    'ideals should have not changed'
  );
  expect(before.get('bonds')).toEqual(
    after.get('bonds'),
    'bonds should have not changed'
  );
});
