...
test('Immutable default character', () => {
  ...
  expect(Map.isMap(state.character)).toEqual(
    true,
    'character should be an Immutable Map'
  );
  expect(List.isList(state.character.get('charAttackBubbles'))).toEqual(
    true,
    'attack bubbles should be an Immutable List'
  );
  expect(state.character.get('charAttackBubbles').size).toEqual(
    0,
    'starts with no attack bubbles'
  );
  expect(state.character.get('charSpellBubbles').size).toEqual(
    0,
    'starts with no spell bubbles'
  );
});
...
test('Adding attack bubble', () => {
  ...
  expect(charAttackBubbles.getIn([0, 'id'])).toEqual(
    'test-attack-bonus',
    'contains the correct id'
  );
  expect(charAttackBubbles.getIn([0, 'abil'])).toEqual(
    'str',
    'contains correct ability mod'
  );
  expect(charAttackBubbles.getIn([0, 'prof'])).toEqual(
    true,
    'contains correct proficiency'
  );
  expect(charAttackBubbles.getIn([0, 'name'])).toEqual(
    'Test Attack Bonus',
    'contains correct name'
  );
  expect(charAttackBubbles.getIn([0, 'bonus'])).toEqual(
    0,
    'contains correct bonus'
  );
  expect(charAttackBubbles.getIn([0, 'score'])).toEqual(
    0,
  ...
  expect(charAttackBubbles.getIn([1, 'id'])).toEqual(
    'test-attack-bonus-2',
    'contains the correct id'
  );
  expect(charAttackBubbles.getIn([1, 'abil'])).toEqual(
    'wis',
    'contains correct ability mod'
  );
  expect(charAttackBubbles.getIn([1, 'prof'])).toEqual(
    false,
    'contains correct proficiency'
  );
  expect(charAttackBubbles.getIn([1, 'name'])).toEqual(
    'Test Attack Bonus 2',
    'contains correct name'
  );
  expect(charAttackBubbles.getIn([1, 'bonus'])).toEqual(
    1,
    'contains correct bonus'
  );
  expect(charAttackBubbles.getIn([1, 'score'])).toEqual(
    1,
    'contains correct score'
  );
  expect(charAttackBubbles.getIn([0, 'id'])).toEqual(
    'test-attack-bonus',
    'retains pushed order'
  );
});
...
test('Updating existing attack bubble', () => {
  ...
  expect(beforeEdit.get('id')).toEqual(
    afterEdit.get('id'),
    'id should remain the same'
  );
  expect(beforeEdit.get('score')).toEqual(0, 'score starts at 0');
  expect(afterEdit.get('score')).toEqual(10, 'score updates to 10');
  expect(beforeEdit.get('prof')).not.toEqual(
    afterEdit.get('prof'),
    'updated prof should take effect'
  );
  expect(beforeEdit.get('name')).not.toEqual(
    afterEdit.get('name'),
    'updated name should take effect'
  );
});
...
test('Updating existing attack bubbles mid-list', () => {
  ...
  expect(charAttackBubbles.getIn([0, 'name'])).toEqual(
    'Test Attack Bonus',
    'first element name should not have changed'
  );
  expect(charAttackBubbles.getIn([1, 'name'])).toEqual(
    'Updated',
    'second attack bonus should be updated'
  );
});
...
test('Deleting attack bubbles', () => {
  ...
  expect(afterDelete.getIn([0, 'id'])).toEqual(
    'test-attack-bonus-1',
    'first item is correct after delete'
  );
  expect(afterDelete.getIn([1, 'id'])).toEqual(
    'test-attack-bonus-3',
    'second item is correct after delete'
  );
});
...
test('Creating Spell Attack Bonuses', () => {
  ...
  expect(afterCreate.getIn([0, 'id'])).toEqual(
    'spell-attack-1',
    'item has correct id'
  );
  expect(afterCreate.getIn([0, 'name'])).toEqual(
    'test-data',
    'item has correct name'
  );
  expect(afterCreate.getIn([0, 'score'])).toEqual(
    10,
    'score gets updated from bonus'
  );
});
...
test('Editing Spell Attack Bonuses', () => {
  ...
  expect(afterEdit.getIn([1, 'id'])).toEqual(
    'spell-attack-2',
    'id still exists'
  );
  expect(afterEdit.getIn([1, 'name'])).toEqual(
    'some name',
    'name got updated correctly'
  );
  expect(afterEdit.getIn([0, 'id'])).toEqual(
    'spell-attack-1',
    'id of first item is unchanged'
  );
  expect(afterEdit.getIn([2, 'id'])).toEqual(
    'spell-attack-3',
    'id of third item is unchanged'
  );
...
test('Deleting Spell Attack Bonuses', () => {
  ...
  expect(afterDelete.getIn([1, 'name'])).toEqual(
    'test-data-3',
    'got correct item name'
  );
  expect(afterDelete.getIn([0, 'id'])).toEqual(
    'spell-attack-1',
    'id of first item is unchanged'
